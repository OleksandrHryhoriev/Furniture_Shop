(() => {
    ...
    let headerNavigation = function() {
        ...
        headerNavMenu.addEventListener("click", closeHeader);
        function closeHeader(e) {
            let headerNavTarget = e.target;
            if (headerNavTarget.closest(".header__link") && document.documentElement.classList.contains("menu-open")) {
                bodyUnlock();
        ...
        document.addEventListener("click", (e => {
            let target = e.target;
            if (!target.closest(".header__nav") && document.documentElement.classList.contains("menu-open")) {
                bodyUnlock();
                document.documentElement.classList.remove("menu-open");
            }
        }));
    ...
    let headerSearch = function() {
        ...
        const searchHeaderBtn = document.querySelector(".search-header__button");
        ...
        searchHeaderBtn.onclick = () => {
            if (searchHeader.classList.contains("_active")) {
                ...
                searchHeaderClose.hidden = true;
            }
            searchHeader.classList.toggle("_active");
        };
        searchHeaderInput.onchange = () => {
            if (searchHeaderInput.value) searchHeaderClose.hidden = false; else searchHeaderClose.hidden = true;
        };
        searchHeaderClose.onclick = () => {
            searchHeaderInput.value = "";
            searchHeaderClose.hidden = true;
        };
    };
    let furnitureSearch = function() {
        const search = document.querySelector(".furniture__search-wrap");
        const searchBtn = document.querySelector(".search-furniture__button");
        const searchInput = document.querySelector(".search-furniture__input");
        const searchClose = document.querySelector(".search-furniture__close");
        searchBtn.onclick = () => {
        ...
        searchInput.onchange = () => {
            if (searchInput.value) searchClose.hidden = false; else searchClose.hidden = true;
        };
        searchClose.onclick = () => {
            searchInput.value = "";
            searchClose.hidden = true;
    ...
    footer.onclick = e => {
        let target = e.target;
        if (target.closest(".footer-spoller__title") && window.innerWidth < 500.98) {
            const targetParent = target.parentElement;
            targetParent.classList.toggle("_active");
        }
    };
    let furnitureTab = function() {
        let activeFurnitureTab = document.querySelector(".tabs-furniture__item._active");
        const labelFurniture = document.querySelector(".tabs-furniture__label");
        if (activeFurnitureTab && labelFurniture) {
            addLabelText(activeFurnitureTab, labelFurniture);
            let activeFurnitureTabText = activeFurnitureTab.innerText.toLowerCase();
            filterProducts(activeFurnitureTabText);
        } else {
            labelFurniture.innerText = "Category";
            filterProducts(null);
        }
        const tabsFurniture = document.querySelector(".tabs-furniture");
        tabsFurniture.addEventListener("click", (e => {
            let target = e.target;
            if (target.closest(".tabs-furniture__label")) tabsFurniture.classList.toggle("_open");
            const tabItems = document.querySelectorAll(".tabs-furniture__item");
            if (target.closest(".tabs-furniture__item")) {
                tabItems.forEach((item => {
                    if (item.classList.contains("_active")) item.classList.remove("_active");
                }));
                let targetItem = target.closest(".tabs-furniture__item");
                targetItem.classList.add("_active");
                addLabelText(targetItem, labelFurniture);
                if (tabsFurniture.classList.contains("_open")) tabsFurniture.classList.remove("_open");
                let targetItemText = targetItem.innerText.toLowerCase();
                filterProducts(targetItemText);
            }
        }));
        document.addEventListener("click", (e => {
            let target = e.target;
            if (!target.closest(".tabs-furniture") && tabsFurniture.classList.contains("_open")) tabsFurniture.classList.remove("_open");
        }));
        function addLabelText(from, to) {
            to.innerText = from.innerText;
        }
        function filterProducts(el) {
            let products = document.querySelectorAll(".product");
            if ("all" === el || null == el) products.forEach((item => item.style.display = "block")); else products.forEach((item => {
                if (item.classList.contains(`_${el}`)) item.style.display = "block"; else item.style.display = "none";
            }));
        }
    };
    headerSearch();
    furnitureSearch();
    furnitureTab();
    let scrollToFunction = function() {
        const navigLinks = document.querySelectorAll("[data-scrollto]");
        if (navigLinks.length > 0) {
            navigLinks.forEach((item => {
                item.addEventListener("click", scrollToNavigation);
            }));
            function scrollToNavigation(e) {
                const navigLink = e.target;
                if (navigLink.dataset.scrollto && document.querySelector(navigLink.dataset.scrollto)) {
                    const scrollToBlock = document.querySelector(navigLink.dataset.scrollto);
